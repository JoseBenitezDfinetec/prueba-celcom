...
# from itertools import filtered
...
MONEDAS = (
    ...
    {'country': u'Estados Unidos', 'currency': 'USD', 'singular': u'DÓLAR', 'plural': u'DÓLARES', 'symbol': u'US$',
     'decimalsingular': u'CENTAVO', 'decimalplural': u'CENTAVOS'},
    ...
    {'country': u'Paraguay', 'currency': 'PYG', 'singular': u'GUARANI', 'plural': u'GUARANIES', 'symbol': u'₲',
     'decimalsingular': u'CENTIMO', 'decimalplural': u'CENTIMOS'},
    {'country': u'Estados Unidos', 'currency': 'DL', 'singular': u'DÓLAR', 'plural': u'DÓLARES', 'symbol': u'US$',
     'decimalsingular': u'CENTAVO', 'decimalplural': u'CENTAVOS'}
)
...
def to_word(number, mi_moneda=None):
    ...
    if mi_moneda != None:
        try:
            ...
            if mi_moneda[:2] == 'DL':
                mi_moneda = 'DL'
            # moneda = filterfalse(lambda x: x['currency'] == mi_moneda, MONEDAS).__next__()
            for moneda in MONEDAS:
                if moneda['currency'] == mi_moneda:
                    break
            if int(number) == 1:
